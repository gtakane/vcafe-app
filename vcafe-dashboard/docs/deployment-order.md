...
> **売上規模が大きくなったら`RESCAN_DEEP_DAYS`の併用を検討する**（2026-07-29追加）。
> `RESCAN_DAYS`を毎時大きくすると「N日×24回/日」の冗長読み取りが売上規模に比例して
> 増え続ける。`RESCAN_DAYS=1`（直近1日は毎時再走査）+ `RESCAN_DEEP_DAYS=2`（それより
> 古い2日分は1日1回、既定`RESCAN_DEEP_HOUR_JST=5`時のみ再走査）にすると、
> 同じ3日分をカバーしつつ冗長回数を約64%減らせる。まずは現行の`RESCAN_DAYS=3`で
> 様子を見て、読み取り時間が問題になってきたら切り替える。
