...
export default function Dashboard({ initialData, initialStart, initialEnd, viewer }: { initialData: AnalyticsData; initialStart: string; initialEnd: string; viewer: Viewer }) {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand"><div className="brand-mark">V</div><div><strong>Vあっと</strong><span>ANALYTICS</span></div></div>
    ...
    <main className="main">
      ...
      {view === "overview" && <>
        ...
        <section className="grid-2"><article className="panel"><div className="panel-head"><div><p className="eyebrow">PERFORMANCE TREND</p><h2>ご帰宅数の推移</h2></div><span className="badge">{granularity === "hour" ? "時間別" : granularity === "day" ? "日次" : granularity === "week" ? "週次" : "月次"}</span></div><LineChart points={trend}/></article><article className="panel"><div className="panel-head"><div><p className="eyebrow">POPULAR MAIDS</p><h2>{viewer.role === "admin" ? "人気メイド" : "実績内訳"}</h2></div></div><div className="ranking">{maidStats.slice(0,5).map((m,i)=><div className="rank-row" key={m.id}><span className="rank">{i+1}</span><span className="mini-avatar">{m.avatar}</span><div><strong>{m.name}</strong><small>{m.users}名・{m.perHour.toFixed(2)}件/h</small></div><div className="bar"><i style={{width:`${m.visits/maxMaidVisits*100}%`}}/></div><b>{m.visits}</b></div>)}</div></article></section>
